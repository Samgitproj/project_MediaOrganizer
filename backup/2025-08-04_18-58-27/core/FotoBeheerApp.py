...
class FotoBeheerApp(QtWidgets.QMainWindow):
    ...
    def start_search_from_location(self):
        ...
        foto_count, video_count = self.tel_media_inclusief_submappen(folder)
    ...
    def tel_enkel_bestanden_in_map(self, pad: str) -> tuple[int, int]:
        fotos = 0
        videos = 0
        try:
    ...
    def laad_subfolders(self, ouder_item: QtWidgets.QTreeWidgetItem, pad: str):
        try:
            for naam in os.listdir(pad):
                ...
                # ✅ Enkel directe bestanden tellen, geen subfolders meenemen
                foto_count, video_count = self.tel_enkel_bestanden_in_map(volledige_map)
                ...
                # Voeg dummy toe zodat ⯈ zichtbaar is als er submappen zijn
                if any(
