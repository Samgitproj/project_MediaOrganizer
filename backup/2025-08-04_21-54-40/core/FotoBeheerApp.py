...
class FotoBeheerApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.ui_dialog.btnSearchAll.clicked.connect(self.exporteer_naar_documentmap)
    ...
    def voeg_map_toe_aan_tree(
        ...
        try:
            ...
            logging.warning(f"Fout bij verwerken van {pad}: {e}")
    ...
    def exporteer_tree_naar_csv(self, bestandsnaam: str):
        import csv

        def schrijf_item(
            item: QtWidgets.QTreeWidgetItem, writer: csv.writer, niveau: int = 0
        ):
            writer.writerow(
                [
                    "  " * niveau + item.text(0),
            ...
            for i in range(item.childCount()):
                schrijf_item(item.child(i), writer, niveau + 1)

        try:
            with open(bestandsnaam, mode="w", newline="", encoding="utf-8") as file:
                writer = csv.writer(file)
                writer.writerow(["Pad", "Foto's", "Video's"])
                for i in range(self.ui_dialog.listFoundedItems.topLevelItemCount()):
                    schrijf_item(
                        self.ui_dialog.listFoundedItems.topLevelItem(i), writer
                    )
            logging.info(f"Mediatree geëxporteerd naar: {bestandsnaam}")
        except Exception as e:
            logging.error(f"Fout bij exporteren van mediatree: {e}")

    def exporteer_naar_documentmap(self):
        pad = r"C:\\OneDrive\\Vioprint\\OneDrive - Vioprint\\software projecten\\MediaOrganizer\\documents\\mediatree_export.csv"
        self.exporteer_tree_naar_csv(pad)
